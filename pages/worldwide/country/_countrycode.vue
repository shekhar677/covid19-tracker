  ...
  <div class="p-3 sm:py-8 sm:px-6 md:px-12 min-h-screen">
    <div class="mb-6 text-black dark-mode:text-grey-50 flex flex-col sm:flex-row items-start sm:items-center justify-between">
      <p class="mb-5 sm:mb-0 text-xs font-semibold select-none">
        ...
        <span>/</span>
        <span class="ml-1 text-black dark-mode:text-grey-50">{{ countryName }}</span>
      </p>
      <div>
    ...
    <div class="mb-8">
      <h1 class="text-black dark-mode:text-grey-50 text-2xl sm:text-4xl font-semibold">{{ countryName }}</h1>
      <p class="text-black dark-mode:text-grey-50 text-xs2 sm:text-xs">Last updated {{ fromNow(updated_at) }}</p>
    </div>
    <div class="mb-8">
