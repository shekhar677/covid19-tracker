    ...
    <div v-if="loaded">
      <div class="bg-white rounded-card shadow-sm p-2 sm:p-4 md:p-10 md:pb-5 py-6 md:m-3 mt-6">
        <div class="flex flex-col sm:flex-row justify-between items-center mb-8">
          <p class="text-black sm:text-base md:text-xl font-semibold mb-3 sm:mb-0">Daily Cases</p>
...
export default {
  ...
  methods: {
    ...
    xTicks(from, to) {
      ...
      this.chartData.labels = this.date.slice(from, to);
      this.chartData.datasets = [];
      this.chartData.datasets.push({
        label: 'Confirmed',
        data: this.dailyconfirmed.slice(from, to),
        fill: true,
  ...
  mounted() {
    this.$axios.get('https://corona-api.com/timeline')
      .then(res => {
        this.date = []
        this.dailyconfirmed = []
        this.dailydeceased = []
