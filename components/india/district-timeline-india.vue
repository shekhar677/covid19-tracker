...
export default {
  ...
  methods: {
    ...
    xTicks(to, from) {
      ...
      this.chartData.datasets.push({
        label: 'Active',
        data: this.dailyactive.slice(to, from),
        fill: false,
        borderColor: '#FF5CA8',
        borderCapStyle: 'round',
        borderWidth: 3,
      ...
      this.chartData.datasets.push({
        label: 'Recovered',
        data: this.dailyrecovered.slice(to, from),
        fill: false,
        borderColor: '#58DFB6',
        borderCapStyle: 'round',
        borderWidth: 3,
      ...
      this.chartData.datasets.push({
        label: 'Deaths',
        data: this.dailydeceased.slice(to, from),
        fill: false,
        borderColor: '#FF5C5C',
        borderCapStyle: 'round',
        borderWidth: 3,
  ...
  mounted() {
    ...
    this.$axios.get('https://api.covid19india.org/districts_daily.json')
      .then(res => {
        this.date = []
        this.dailyconfirmed = []
        this.dailydeceased = []
        ...
        let daily = res.data.districtsDaily;
        let state = this.state;
        let district = this.district;
        let stateData;
        let manipulatedData;
        for (let i in daily) {
          if (i.toLowerCase() == state.toLowerCase()) {
            for (let d in daily[i]) {
              if (d.toLowerCase() == district.toLowerCase()) {
                manipulatedData = daily[i][d]
              }
            }
          }
        }
        ...
        manipulatedData.map((d, i) => {
          ...
          this.dailyconfirmed.push(d.confirmed);
          this.dailydeceased.push(d.deceased);
          this.dailyrecovered.push(d.recovered);
          this.dailyactive.push(d.active);
        });
